...
proxyServe outside_config inside_config translation_config = do
    ...
    let 
        http2_attendant = http2Attendant sessions_context coherent_worker
        http1_attendant = http11Attendant sessions_context coherent_worker
    ...
    tlsServeWithALPNAndFinishOnRequest
        ...
        [
            ("no-protocol", http1_attendant),
            ("h2", http2_attendant),
            ("h2-17", http2_attendant),
            ("h2-15", http2_attendant),
            ("h2-14", http2_attendant),
            ("http/1.1", http1_attendant)
        ]
        (pair_ip_port ^. _2 )
        finish_request
...
route vh_list req@(headers, _) = let 
    ...
    requested_host = case lookup ":authority" headers  of 
        Nothing -> case lookup "host" headers of 
            Just host -> host 
            Nothing -> "*"
        Just host -> host 
